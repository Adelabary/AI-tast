...
def heuristic(state):
    # Heuristic: estimated number of moves required to reach the goal state
    return state.left_m + state.left_c + state.boat
        ...
        for next_state in next_states:
            if next_state not in visited:
                ...
                if next_state.is_goal():
                    return new_path[1:]  # Exclude the initial state from the path
