...
def dls(current_state, visited, depth_limit, path):
    ...
    for next_state in next_states:
        if next_state not in visited:
            result = dls(next_state, visited, depth_limit - 1, path + [next_state])
            if result:
...
def main():
    ...
    solution = dls(initial_state, visited, depth_limit, path)
    if solution:
